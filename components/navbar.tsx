...
const Navbar: React.FC<{
  className?: string | ((inView: boolean) => string);
}> = ({ className }) => {
  ...
  return (
    <motion.nav
      ...
      className={cn(
        "px-8 lg:px-24 py-4 sticky z-30 lg:mx-auto bg-white",
        !inView &&
          "md:px-8 rounded-[2rem] max-w-screen-lg mx-4 py-2 bg-white/80 shadow-md backdrop-blur-sm",
...
const Nav: React.FC<{ fullUrl: string | null; mobile: boolean }> = ({
  fullUrl,
  mobile,
}) => {
    ...
    >
      {navlinks.map(({ label, href }) => (
        ...
        >
          <a
            className={cn(
              "p-2 border-b-2 border-transparent block text-center whitespace-nowrap opacity-60",
              fullUrl == href && "text-primary"
            )}
            href={href}
          >
            {label}
          ...
          <motion.div
            ...
            className={cn(
              "absolute h-0.5 bg-current top-full",
              fullUrl == href && "text-primary"
            )}
          ></motion.div>
        </ListItemElement>
